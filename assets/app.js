(() => {
  ...
  const themePref = localStorage.getItem('sd_theme');
  if (themePref === 'light') document.body.classList.add('theme-light');
  ...
  function isPopular(deal) {
    if (state.popular.size === 0) return false;
  ...
  function isFree(deal) {
    ...
    return (
      p &&
      (p.final === 0 ||
        /free|бесплат|0[,.]00/i.test(String(p.final_formatted || '')))
    );
  }
  ...
  function list() {
    ...
    arr = [...arr];
    const popFirst = (a, b) => (isPopular(b) ? 1 : 0) - (isPopular(a) ? 1 : 0);
  ...
  function cardHtml(d, sizeClass, compact) {
    ...
    if (price && price.final_formatted) {
      priceHtml =
        ...
        (price.original_formatted
          ? '<span class="p-old">' + esc(price.original_formatted) + '</span>'
          : '') +
        (price._fb
          ? '<span class="p-note">(' +
    ...
    const media =
      ...
      d.discount +
      '%</span>' +
      (pop ? '<span class="badge-top">изв.</span>' : '') +
      '</div>';
    ...
    return (
      ...
      '">' +
      media +
      '<div class="tile-body"><h3 class="tile-name">' +
      esc(d.name || 'Без названия') +
      '</h3><div class="price-line">' +
      priceHtml +
      '</div>' +
      (!compact && multi
  ...
  function renderFree() {
    ...
    els.freeRow.innerHTML = free
      .map((d, i) => cardHtml(d, i === 0 ? 'tile-lg' : 'tile-md', true))
      .join('');
    els.freeRow.querySelectorAll('.tile').forEach((el) => {
  ...
  function render() {
    ...
    if (els.edition) {
      els.edition.textContent =
        'вып. · ' + (state.updatedAt ? state.updatedAt.slice(0, 10) : 'specials');
    }

    let topPool = state.deals.filter((d) => (d.discount || 0) >= 40);
    if (state.popular.size) {
      const pop = topPool.filter(isPopular);
      if (pop.length >= 2) topPool = pop;
    }
    topPool = [...topPool]
      .sort(
        (a, b) =>
          (isPopular(b) ? 1 : 0) - (isPopular(a) ? 1 : 0) || b.discount - a.discount
      )
      .slice(0, 4);
    if (els.topRow) {
      els.topRow.innerHTML = topPool
        .map((d, i) =>
          cardHtml(d, i === 0 ? 'tile-xl' : i === 1 ? 'tile-lg' : 'tile-md', true)
        )
        .join('');
      els.topRow.querySelectorAll('.tile').forEach((el) => {
        el.style.gridColumn = 'span 1';
      });
    ...
    if (els.grid) {
      els.grid.innerHTML = slice
        .map((d, i) => cardHtml(d, tileSize(d, i), false))
        .join('');
    ...
    if (els.moreBtn) {
      ...
      els.moreBtn.textContent =
        left > 0 ? 'ещё ' + left + ' из архива' : 'всё';
    }
  }

  function syncChips() {
    if (els.discChips) {
      els.discChips.querySelectorAll('.chip').forEach((btn) => {
        btn.classList.toggle('on', Number(btn.dataset.min) === state.minDiscount);
      });
    }
    if (els.priceChips) {
      els.priceChips.querySelectorAll('.chip').forEach((btn) => {
  ...
  function resetAll() {
    ...
    state.maxPrice = 0;
    state.sort = 'discount';
    state.onlyPopular = true;
    state.onlyPriced = false;
  ...
  async function load() {
    ...
    try {
      ...
      const raw = Array.isArray(data.deals) ? data.deals : [];
      const byId = new Map();
      for (const d of raw) {
        const id = Number(d.appid);
        if (!id) continue;
        const prev = byId.get(id);
        if (!prev || (d.discount || 0) > (prev.discount || 0)) byId.set(id, d);
      }
  ...
  on(els.maxPrice, 'change', () => {
    state.maxPrice = Number(els.maxPrice.value) || 0;
    state.shown = state.pageSize;
    persist();
    syncChips();
    render();
  });
  on(els.onlyPriced, 'change', () => {
    state.onlyPriced = els.onlyPriced.checked;
    persist();
    render();
  });
  on(els.onlyPopular, 'change', () => {
    state.onlyPopular = els.onlyPopular.checked;
    state.shown = state.pageSize;
    persist();
    render();
  });
  on(els.onlyFree, 'change', () => {
    state.onlyFree = els.onlyFree.checked;
    state.shown = state.pageSize;
    persist();
    render();
  });
  on(els.hideUnknown, 'change', () => {
    state.hideUnknown = els.hideUnknown.checked;
    persist();
    render();
  });
  ...
  on(els.priceChips, 'click', (e) => {
    const btn = e.target.closest('.chip');
    if (!btn) return;
    state.maxPrice = Number(btn.dataset.max) || 0;
    if (els.maxPrice) els.maxPrice.value = String(state.maxPrice);
    state.shown = state.pageSize;
    persist();
    syncChips();
  ...
  on(themeBtn, 'click', () => {
    ...
    localStorage.setItem('sd_theme', light ? 'light' : 'dark');
  });
